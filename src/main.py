...
PAYMENTS_TOKEN = settings.payment_token()

SUPERSPEED_SHIPPING_OPTION = ShippingOption(
    id='cdek',
    title='CDEK'
).add(LabeledPrice('Отправка CDEK\'ом', 60000))

POST_SHIPPING_OPTION = ShippingOption(
    id='post',
    title='Почта России'
).add(LabeledPrice('Отправка почтой России', 31500))
...
async def shipping_process(shipping_query: ShippingQuery):
    if shipping_query.shipping_address.country_code != 'RU':
        return await bot.answer_shipping_query(
            shipping_query.id,
            ok=False,
            error_message='Заказывать можно только в Россию.'
        )

    shipping_options = [SUPERSPEED_SHIPPING_OPTION, POST_SHIPPING_OPTION]
...
async def successful_payment(msg: Message):
    order=ordr.Order(msg.successful_payment.invoice_payload)
    order.set_home_adress(f"{msg.successful_payment.order_info.shipping_address.post_code}, {msg.successful_payment.order_info.shipping_address.city}, {msg.successful_payment.order_info.shipping_address.street_line1}, {msg.successful_payment.order_info.shipping_address.street_line2}")
    google.set_address(order.get_order_id(), f"{msg.successful_payment.order_info.shipping_address.post_code}, {msg.successful_payment.order_info.shipping_address.city}, {msg.successful_payment.order_info.shipping_address.street_line1}, {msg.successful_payment.order_info.shipping_address.street_line2}")
    await bot.send_message(
        msg.chat.id,
        f"Заказ #{msg.successful_payment.invoice_payload} успешно оплачен. Спасибо за покупку. Менеджер свяжется с вами в ближайшее время."
    )
...
async def process_callback(callback_query: types.CallbackQuery):
    ...
    if call_data.startswith("admin_") and user.is_admin():
        ...
        # Settings
            ...
            if call_data[24:] == "Default":
                    value = 1
            if call_data[24:] == "Add":
                    value = int(settings.get_borderwidth()) + 1
            if call_data[24:] == "Reduce":
                    value = int(settings.get_borderwidth()) - 1
            ...
            if call_data[26:] == "Default":
                    value = 16
            if call_data[26:] == "Add":
                    value = int(settings.get_titlefontsize()) + 2
            if call_data[26:] == "Reduce":
                    value = int(settings.get_titlefontsize()) - 2
            ...
            if call_data[25:] == "Default":
                    value = 16
            if call_data[25:] == "Add":
                    value = int(settings.get_axisfontsize()) + 2
            if call_data[25:] == "Reduce":
                    value = int(settings.get_axisfontsize()) - 2
...
async def checkoutCartSetAdditionalMessage(message: types.Message, state: FSMContext):
    ...
    if settings.is_captcha_enabled():
        ...
        await bot.send_message(
            chat_id=message.chat.id,
            text=tt.get_order_confirmation_template(item_amount_dict=user.get_cart_amount(), cart_price=user.get_cart_price(), email_adress=data["email"], additional_message=message.text, phone_number=data["phone_number"] if settings.is_phone_number_enabled() else None, home_adress=None),
...
async def checkoutCartCheckCaptcha(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == data["captcha"].lower():
        await bot.send_message(
            chat_id=message.chat.id,
            text=tt.get_order_confirmation_template(item_amount_dict=user.get_cart_amount(), cart_price=user.get_cart_price(), email_adress=data["email"], additional_message=data["additional_message"], phone_number=data["phone_number"] if settings.is_phone_number_enabled() else None, home_adress=None),
            reply_markup=markups.get_markup_checkoutCartConfirmation(),
        )
        await state_handler.checkoutCart.confirmation.set()
    else:
...
async def cancelState(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if call_data[:6] == "admin_":
        ...
        if call_data == "catalogue":
            ...
            try:
                order = ordr.create_order(order_id, user_id, item_list_comma, email, additional_message, phone_number=phone_number)
                user.clear_cart()
                text = f"Заказ с ID {order.get_order_id()} был успешно создан.\nСпасибо за заказ! Наш менеджер свяжется с вами в ближайшее время."
                items=', '.join([f"{item.get_name()} - {item.get_price()}₽" for item in order.get_item_list()])
                prices=[LabeledPrice(label=f'Заказ #{order.get_order_id()}', amount=int(order.get_item_list_price()*100))]
                await bot.send_invoice(
                    chat_id=chat_id,
                    title=f'заказ #{order.get_order_id()}',
                    description=items,
